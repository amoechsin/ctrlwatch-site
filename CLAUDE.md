...
`issues.js` update → `npm run covers` → `npm run inject:og` → `npm run inject:seo` → `npm run inject:top50link` → `npm run inject:reviewlinks` → `npm run inject:shell` → update tracker → `npm run build:creators` → `npm run build:search` → commit.
