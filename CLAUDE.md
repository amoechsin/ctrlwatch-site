...
**Search:**
- `public/search-index.json` generated by `scripts/build-search-index.mjs` from `issues.js`, `creators.json`, and the tracker (Time Capsule subjects, Boss Fights, Special Features). Run `npm run build:search` after either is updated. NOT inlined — `<Search />` fetches `/search-index.json` once, on first search open (the index grows every issue; inlining it shipped ~53KB in every page).
